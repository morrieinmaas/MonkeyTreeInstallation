 ...
 * @since       2.5
 */
class plgFinderEasySocialProjects extends FinderIndexerAdapter
{
	/**
	 ...
	 * @since  2.5
	 */
	protected $context = 'EasySocial.Projects';
	 ...
	 * @since  2.5
	 */
	protected $type_title = 'EasySocial.Projects';
	...
	{
		if ($context == 'easysocial.projects')
		{
			$id = $table->id;
			...
			$query = "select `link_id` from `#__finder_links` where `url` like '%option=com_easysocial&view=projects&id=$id%'";
			$sql->raw($query);
			$db->setQuery($sql);
			$item = $db->loadResult();
	...
	protected function index(FinderIndexerResult $item, $format = 'html')
	{
		...
		if ($item->type == SOCIAL_PROJECT_TYPE_PRIVATE) {
			$access = 2;
		}
		...
		$project = FD::project( $item->id );

		// Build the necessary route and path information.
		// index.php?option=com_easysocial&view=groups&id=7:chogokin&layout=item&Itemid=799

		$item->url 		= 'index.php?option=com_easysocial&view=projects&id=' . $project->getAlias() . '&layout=item';

		$item->route	= $project->getPermalink();
		$item->route 	= $this->removeAdminSegment($item->route);
		$item->path 	= FinderIndexerHelper::getContentPath($item->route);

		$item->access		= $access;
		$item->alias		= $project->getAlias();
		$item->state		= 1;
		$item->catid		= $project->category_id;
		$item->start_date	= $project->created;
		$item->created_by	= $project->creator_uid;
		$item->created_by_alias	= $userAlias;
		$item->modified		= $project->created;
		$item->modified_by	= $project->creator_uid;
		$item->params		= '';
		$item->metakey		= $item->category . ' ' . $project->title;
		$item->metadesc		= $project->title . ' ' . $project->description;
		$item->metadata		= '';
		$item->publish_start_date	= $project->created;
		$item->category		= $item->category;
		$item->cat_state	= 1;
		$item->cat_access	= 0;

		$item->summary          = $project->title . ' ' . $project->description;
		$item->body 			= $project->title . ' ' . $project->description;
		...
		$registry->set( 'image' , $project->getAvatar() );
		...
		$item->addTaxonomy('Type', 'EasySocial.Projects');
	...
	protected function getListQuery($sql = null)
	{
		...
		$sql->where( 'a.state = 1');
		$sql->where( 'a.cluster_type = ' . $db->Quote( 'project' ) );
