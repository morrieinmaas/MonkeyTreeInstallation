...
class EasySocialViewProjects extends EasySocialSiteView
{
    ...
    public function renderCalendar()
    {
        ...
        $startOfWeek = $this->config->get('projects.startofweek');
        ...
        $calendar->header = FD::date($unix)->toFormat(JText::_('COM_EASYSOCIAL_DATE_MY'));
        ...
        // Create a date range to retrieve all the projects
        $start = $year . '-' . $month . '-' . '01 00:00:00';
        $end = $year . '-' . $month . '-' . $calendar->days_in_month . ' 23:59:59';

        $projects = FD::model('Projects')->getProjects(array(
        ...
        // This array groups the projects by days
        $days = array();

        foreach($projects as $project) {
            $days[$project->getProjectStart()->format('j', true)][] = $project;
        }
        ...
        $output = $theme->output('site/projects/default.calendar');
    ...
    public function getProjects($filter, $projects, $pagination, $activeCategory, $featuredProjects)
    {
        $theme = FD::themes();
        ...
        $theme->set('featuredProjects', $featuredProjects);
            ...
            switch ($mode) {
                ...
                case 3:
                    ...
                    if ($nextDate == $currentDay) {
                        ...
                        $nextTitle = JText::_('COM_EASYSOCIAL_PAGE_TITLE_PROJECTS_FILTER_TOMORROW') . ' - ' . FD::date($nextDate, false)->format(JText::_($dateFormat));
        ...
        if ($filter === 'nearby') {
            ...
            $resolveOptions['title'] = JText::sprintf('COM_EASYSOCIAL_PROJECTS_IN_DISTANCE_RADIUS', $distance, $distanceUnit);
        }
        ...
        $hrefs['start'] = array(
            'nopast' => FRoute::projects($routeOptions, false)
        );

        if ($showSorting) {
            // Only need to create the "order by created" link.
            $hrefs['created'] = array(
                'nopast' => FRoute::projects(array_merge($routeOptions, array('ordering' => 'created')), false)
            );
        }
        ...
        if ($showDistanceSorting) {
            $hrefs['distance'] = array(
                'nopast' => FRoute::projects(array_merge($routeOptions, array('ordering' => 'distance')), false)
            );
        }

        if ($showPastFilter) {
            // If past filter is displayed on the page, then we need to generate the past links counter part
            $hrefs['start']['past'] = FRoute::projects(array_merge($routeOptions, array('includePast' => 1)), false);

            if ($showSorting) {
                // Only need to create the "order by created" link.
                $hrefs['created']['past'] = FRoute::projects(array_merge($routeOptions, array('ordering' => 'created', 'includePast' => 1)), false);
            }

            if ($showDistanceSorting) {
                // Only need to create the "order by distance" link.
                $hrefs['distance']['past'] = FRoute::projects(array_merge($routeOptions, array('ordering' => 'distance', 'includePast' => 1)), false);
            }
        }
        ...
        if ($sort) {
            $content = $theme->output('site/projects/default.list.items');
        } else {
            $content = $theme->output('site/projects/default.list');
        }
    ...
    public function confirmPromoteGuest()
    {
        ...
        // Get the current user as a guest object in the same project
        $myGuest = FD::table('ProjectGuest');
        ...
        if ($my->isSiteAdmin() || $myGuest->isAdmin()) {
            ...
            $contents = $theme->output('site/projects/dialog.guest.promote');

            return $this->ajax->resolve($contents);
        }

        return $this->ajax->resolve($theme->output('site/projects/dialog.guest.error'));
    }
    ...
    public function confirmDemoteGuest()
    {
        ...
        if (($my->isSiteAdmin() || $myGuest->isOwner()) && $guest->isStrictlyAdmin()) {
            ...
            $contents = $theme->output('site/projects/dialog.guest.demote');
    ...
    public function confirmApproveGuest()
    {
        ...
        if (($my->isSiteAdmin() || $myGuest->isAdmin()) && $guest->isPending()) {
            ...
            $contents = $theme->output('site/projects/dialog.guest.approve');
    ...
    public function confirmRejectGuest()
    {
        ...
        if (($my->isSiteAdmin() || $myGuest->isAdmin()) && $guest->isPending()) {
            ...
            $contents = $theme->output('site/projects/dialog.guest.reject');
    ...
    public function confirmRemoveGuest()
    {
        ...
        if (!$guest->isOwner() && ($my->isSiteAdmin() || $myGuest->isOwner() || ($myGuest->isAdmin() && !$guest->isAdmin()))) {
            ...
            $contents = $theme->output('site/projects/dialog.guest.remove');
    ...
    public function getFilter($project = null, $filter = null)
    {
        if ($this->hasErrors()) {
            return $this->ajax->reject($this->getMessage());
        }
    ...
    public function getAppContents($app)
    {
        ...
        $contents = $lib->renderView(SOCIAL_APPS_VIEW_TYPE_EMBED, 'projects', $app, array('projectId' => $project->id));
    ...
    public function getStream($stream = null)
    {
        ...
        // Get the project id from request
        ...
        $contents = $theme->output('site/projects/item.feeds');
    ...
    public function inviteFriendsDialog()
    {
        ...
        if (empty($project) || empty($project->id)) {
            return $this->ajax->reject(JText::_('COM_EASYSOCIAL_PROJECTS_INVALID_PROJECT_ID'));
        }

        if (!$project->isPublished()) {
            return $this->ajax->reject(JText::_('COM_EASYSOCIAL_PROJECTS_PROJECT_UNAVAILABLE'));
        }

        $guest = $project->getGuest($this->my->id);

        if (!$this->my->isSiteAdmin() && $project->isInviteOnly() && !$guest->isParticipant()) {
            return $this->ajax->reject(JText::_('COM_EASYSOCIAL_PROJECTS_NO_ACCESS_TO_PROJECT'));
        }
        ...
        $contents = $theme->output('site/projects/dialog.inviteFriends');
    ...
    public function inviteFriends()
    {
        ...
        return $this->ajax->resolve(JText::_('COM_EASYSOCIAL_PROJECTS_SUCCESSFULLY_INVITED_FRIENDS'));
    }
    ...
    public function notGoingDialog()
    {
        ...
        $theme = FD::themes();
        $theme->set('project', $project);
        $contents = $theme->output('site/projects/dialog.user.notgoing');
    ...
    public function itemActionDialog()
    {
        ...
        switch($action) {
            case 'feature':
                $btnLabel = JText::_('COM_EASYSOCIAL_PROJECTS_FEATURE_THIS_PROJECT');
                break;
            default:
                $btnLabel = JText::_('COM_EASYSOCIAL_YES_BUTTON');
                break;
        ...
        // COM_EASYSOCIAL_PROJECTS_DIALOG_FEATURE_PROJECT_TITLE
        // COM_EASYSOCIAL_PROJECTS_DIALOG_FEATURE_PROJECT_CONTENT
        // COM_EASYSOCIAL_PROJECTS_DIALOG_DELETE_PROJECT_TITLE
        // COM_EASYSOCIAL_PROJECTS_DIALOG_DELETE_PROJECT_CONTENT
        // COM_EASYSOCIAL_PROJECTS_DIALOG_UNFEATURE_PROJECT_TITLE
        // COM_EASYSOCIAL_PROJECTS_DIALOG_UNFEATURE_PROJECT_CONTENT
        // COM_EASYSOCIAL_PROJECTS_DIALOG_UNPUBLISH_PROJECT_TITLE
        // COM_EASYSOCIAL_PROJECTS_DIALOG_UNPUBLISH_PROJECT_CONTENT

        $contents = $theme->output('site/projects/dialog.item.action');

        return $this->ajax->resolve($contents);
    }

    /**
     * Updates the guest state for a particular project
     *
     * @since   1.3
     * @access  public
     * @param   string
    ...
    public function refreshGuestState()
    {
        ...
        $contents = $theme->output('site/projects/guestState.content');
    ...
    public function withdrawDialog()
    {
        ...
        $theme = FD::themes();
        $theme->set('project', $project);
        $contents = $theme->output('site/projects/dialog.user.withdraw');
    ...
    public function requestDialog()
    {
        ...
        $theme = FD::themes();
        $theme->set('project', $project);
        $contents = $theme->output('site/projects/dialog.user.request');

        return $this->ajax->resolve($contents);
    }

    public function unpublishProjectDialog()
    {
        FD::requireLogin();
        ...
        $theme = FD::themes();
        $theme->set('project', $project);
        $contents = $theme->output('site/projects/dialog.project.unpublish');

        return $this->ajax->resolve($contents);
    }

    public function deleteProjectDialog()
    {
        FD::requireLogin();
        ...
        if (!$guest->isOwner() && !$this->my->isSiteAdmin() && (!$project->isGroupProject() || ($project->isGroupProject() && !$project->getGroup()->isOwner()))) {
            return $this->ajax->reject(JText::_('COM_EASYSOCIAL_PROJECTS_NO_ACCESS_TO_PROJECT'));
        }
        ...
        // Recurring support
        if ($project->isRecurringProject() || $project->hasRecurringProjects()) {
            $contents = $theme->output('site/projects/dialog.project.deleteRecurring');
        } else {
            $contents = $theme->output('site/projects/dialog.project.delete');
        }

        return $this->ajax->resolve($contents);
    }

    public function deleteFilter($projectId)
    {
        $ajax = FD::ajax();
        ...
        $project = FD::project($projectId);
        $url = FRoute::projects(array('layout' => 'item', 'id' => $project->getAlias()), false);
    ...
    public function deleteRecurringDialog()
    {
        ...
        $theme = FD::themes();
        $theme->set('project', $project);
        $contents = $theme->output('site/projects/dialog.recurringproject.delete');
    ...
    /**
     * Updates the project button state
     *
     * @since   1.4
     * @access  public
     * @param   string
    ...
    public function refreshButtonState()
    {
        ...
        $contents = $project->showRsvpButton($isPopbox);
