...
class SocialProjectAppDiscussionsHookNotificationComments
{
    public function execute(&$item)
    {
        ...
        if ($item->context_type === 'discussions.project.create') {
            ...
            if ($isOwner) {
                $string = FD::string()->computeNoun('APP_PROJECT_DISCUSSIONS_USER_COMMENTED_ON_YOUR_DISCUSSION', count($users));
                $item->title = JText::sprintf($string, $names, $project->getName());
            } else {
                $string = FD::string()->computeNoun('APP_PROJECT_DISCUSSIONS_USER_COMMENTED_ON_USERS_DISCUSSION', count($users));
        ...
        if ($item->context_type === 'discussions.project.reply') {
            if ($isOwner) {
                $string = FD::string()->computeNoun('APP_PROJECT_DISCUSSIONS_USER_COMMENTED_ON_YOUR_REPLY', count($users));
                $item->title = JText::sprintf($string, $names, $project->getName());
            } else {
                $string = FD::string()->computeNoun('APP_PROJECT_DISCUSSIONS_USER_COMMENTED_ON_USERS_REPLY', count($users));
                $item->title = JText::sprintf($string, $names, FD::user($discussion->created_by)->getName(), $project->getName());
            }
        }
    }
