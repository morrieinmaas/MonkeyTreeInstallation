...
class DiscussionsControllerReply extends SocialAppsController
{
    ...
    public function accept()
    {
        ...
        // Get the project
        $project = FD::project($reply->uid);

        // Check if the viewer can accept this reply as an answer.
        if (!$project->getGuest()->isAdmin() && $this->my->id != $discussion->created_by && !$this->my->isSiteAdmin()) {
            return $ajax->reject();
        }
        ...
        // @points: projects.discussion.answer
        // Earn points when a reply is marked as an answer
        $points = FD::points();
        $points->assign('projects.discussion.answer', 'com_easysocial', $reply->created_by);
        ...
        // Set the params to cache the project data
        ...
        $options = array();
        $options['permalink'] = FRoute::apps(array('layout' => 'canvas', 'customView' => 'item', 'uid' => $project->getAlias(), 'type' => SOCIAL_TYPE_PROJECT, 'id' => $this->getApp()->getAlias(), 'discussionId' => $discussion->id, 'external' => true), false);
        ...
        $project->notifyMembers('discussion.answered', $options);
    ...
    public function delete()
    {
        ...
        if (!$project->getGuest()->isAdmin() && !$this->my->isSiteAdmin()) {
            return $ajax->reject();
        }
    ...
    public function confirmDelete()
    {
        ...
        $theme = FD::themes();
        $output = $theme->output('apps/project/discussions/canvas/dialog.delete');
    ...
    public function submit()
    {
        ...
        if (!$project->getGuest()->isGuest()) {
            $obj = new stdClass();
            $obj->message = JText::_('APP_PROJECT_DISCUSSIONS_DISCUSSION_IS_AVAILABLE_TO_GUESTS_ONLY');
            $obj->type = SOCIAL_MSG_ERROR;
            return $ajax->reject($obj);
        }

        // Test for locked discussion.
        if ($discussion->lock && !$project->isAdmin()) {
            $obj = new stdClass();
            $obj->message = JText::_('APP_PROJECT_DISCUSSIONS_DISCUSSION_IS_LOCKED');
            $obj->type = SOCIAL_MSG_ERROR;
        ...
        if (empty($content)) {
            $obj = new stdClass();
            $obj->message = JText::_('APP_PROJECT_DISCUSSIONS_EMPTY_REPLY_ERROR');
            $obj->type = SOCIAL_MSG_ERROR;
        ...
        if (!$id) {
            // @points: projects.discussion.reply
            // Earn points when posting a reply
            $points = FD::points();
            $points->assign('projects.discussion.reply', 'com_easysocial', $reply->created_by);
        }
        ...
        $contents = $theme->output('apps/project/discussions/canvas/item.reply');
        ...
        $project->notifyMembers('discussion.reply', $options);
