...
class SocialProjectAppNewsHookNotificationComments
{
    /**
    ...
    public function execute(&$item)
    {
        ...
        if ($item->context_type == 'news.project.create') {
            ...
            if ($news->created_by == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $item->title = JText::sprintf('APP_PROJECT_NEWS_USER_COMMENTED_ON_YOUR_ANNOUNCEMENT', $names, $project->getName());

                return $item;
            }

            // This is for 3rd party viewers
            $item->title = JText::sprintf('APP_PROJECT_NEWS_USER_COMMENTED_ON_USERS_ANNOUNCEMENT', $names, FD::user($news->created_by)->getName(), $project->getName());
