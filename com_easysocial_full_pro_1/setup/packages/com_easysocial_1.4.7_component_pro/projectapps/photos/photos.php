...
class SocialProjectAppPhotos extends SocialAppItem
{
    /**
    ...
    public function onNotificationLoad(SocialTableNotification &$item)
    {
        ...
        $allowedContexts = array('photos.project.upload', 'stream.project.upload', 'photos.project.add', 'albums.project.create', 'photos.project.uploadAvatar', 'photos.project.updateCover');
    ...
    public function getFavIcon()
    {
        ...
        $obj->color = '#F8829C';
        $obj->icon = 'fa-image';
        $obj->label = 'APP_PROJECT_PHOTOS_STREAM_TOOLTIP';
    ...
    public function onPrepareStream(SocialStreamItem &$item)
    {
        ...
        $item->color = '#F8829C';
        $item->label = FD::_('APP_PROJECT_PHOTOS_STREAM_TOOLTIP', true);
    ...
    private function processActions(SocialStreamItem &$item)
    {
        ...
        $repost = FD::get('Repost', $item->uid, SOCIAL_TYPE_STREAM, SOCIAL_APPS_GROUP_PROJECT);
        $item->repost = $repost;
        ...
        if ($item->verb =='share') {
            ...
            $likes = FD::likes();
            $likes->get($objectId, $objectType, 'upload', SOCIAL_APPS_GROUP_PROJECT, $item->uid);
            $item->likes    = $likes;

            // Append the comment action on the stream
            $comments = FD::comments($objectId, $objectType, 'upload', SOCIAL_APPS_GROUP_PROJECT,  array('url' => $commentUrl), $item->uid);
    ...
    public function onAfterSave(&$data)
    {
        ...
        // for groups, projects and etc.. the source will determine the type.
    ...
    public function onPrepareStoryPanel($story)
    {
        ...
        $project = ES::project($story->cluster);
        ...
        if ($access->exceeded('photos.max', $project->getTotalPhotos())) {
        ...
        if ($access->exceeded('photos.maxdaily', $project->getTotalPhotos(true))) {
        ...
        $script->set('type', SOCIAL_TYPE_PROJECT);
        $script->set('uid', $project->id);
    ...
    public function onAfterLikeSave(&$likes)
    {
        ...
        $allowed = array('photos.project.upload', 'stream.project.upload', 'albums.project.create', 'photos.project.add', 'photos.project.uploadAvatar', 'photos.project.updateCover');
        ...
        if ($likes->type == 'albums.project.create') {
            ...
            $emailOptions = array(
                'title' => 'APP_PROJECT_PHOTOS_EMAILS_LIKE_ALBUM_ITEM_SUBJECT',
                'template' => 'apps/project/photos/like.album.item',
            ...
            $recipients = $this->getStreamNotificationTargets($likes->uid, 'albums', 'project', 'create', array(), array($album->user_id, $likes->created_by));

            $emailOptions['title'] = 'APP_PROJECT_PHOTOS_EMAILS_LIKE_ALBUM_INVOLVED_SUBJECT';
            $emailOptions['template'] = 'apps/project/photos/like.album.involved';
        ...
        $allowed = array('photos.project.upload', 'stream.project.upload', 'photos.project.add', 'photos.project.uploadAvatar', 'photos.project.updateCover');
        if (in_array($likes->type, $allowed)) {
            ...
            // Set the email options
            $emailOptions = array(
                'template' => 'apps/project/photos/like.photo.item',
                'actor' => $actor->getName(),
                'actorAvatar' => $actor->getAvatar(SOCIAL_AVATAR_SQUARE),
                'actorLink' => $actor->getPermalink(true, true)
           );
            ...
            $ownerTitle = JText::_('APP_PROJECT_PHOTOS_EMAILS_LIKE_PHOTO_ITEM_SUBJECT');
            $involvedTitle = JText::_('APP_PROJECT_PHOTOS_EMAILS_LIKE_PHOTO_INVOLVED_SUBJECT');

            // If this item is multiple share on the stream, we need to get the photo id here.
            if ($likes->type == 'stream.project.upload') {
            ...
            // For single photo items on the stream
            if ($likes->type == 'photos.project.upload' || $likes->type == 'photos.project.add' || $likes->type == 'photos.project.uploadAvatar' || $likes->type == 'photos.project.updateCover') {
                // Get the photo object
                $photo = FD::table('Photo');
            ...
            if ($likes->type == 'photos.project.uploadAvatar') {
                $verb = 'uploadAvatar';

                $ownerTitle = 'APP_PROJECT_PHOTOS_EMAILS_LIKE_PROFILE_PICTURE_ITEM_SUBJECT';
                $involvedTitle = 'APP_PROJECT_PHOTOS_EMAILS_LIKE_PROFILE_PICTURE_INVOLVED_SUBJECT';
            }

            if ($likes->type == 'photos.project.updateCover') {
                $verb = 'updateCover';

                $ownerTitle = 'APP_PROJECT_PHOTOS_EMAILS_LIKE_PROFILE_COVER_ITEM_SUBJECT';
                $involvedTitle = 'APP_PROJECT_PHOTOS_EMAILS_LIKE_PROFILE_COVER_INVOLVED_SUBJECT';
            }
            ...
            // Get a list of recipients to be notified for this stream item
            // We exclude the owner of the note and the actor of the like here
            $recipients = $this->getStreamNotificationTargets($likes->uid, $element, 'project', $verb, $additionalRecipients, array($photo->user_id, $likes->created_by));

            $emailOptions['title'] = $involvedTitle;
            $emailOptions['template'] = 'apps/project/photos/like.photo.involved';
    ...
    {
        $allowed = array('photos.project.upload', 'albums.project.create', 'stream.project.upload', 'photos.project.add', 'photos.project.uploadAvatar', 'photos.project.updateCover');
        ...
        // For likes on albums when user uploads multiple photos within an album
        if ($comment->element == 'albums.project.create') {
            ...
            // Set the email options
            $emailOptions = array(
                'title' => 'APP_PROJECT_PHOTOS_EMAILS_COMMENT_ALBUM_ITEM_SUBJECT',
                'template' => 'apps/project/photos/comment.album.item',
                'permalink' => $album->getPermalink(true, true),
            ...
            // Get a list of recipients to be notified for this stream item
            // We exclude the owner of the note and the actor of the like here
            $recipients = $this->getStreamNotificationTargets($comment->uid, 'albums', 'project', 'create', array(), array($album->user_id, $comment->created_by));

            $emailOptions['title'] = 'APP_PROJECT_PHOTOS_EMAILS_COMMENT_ALBUM_INVOLVED_SUBJECT';
            $emailOptions['template'] = 'apps/project/photos/comment.album.involved';
        ...
        // Set the email options
        $emailOptions = array(
            'template' => 'apps/project/photos/comment.photo.item',
            'actor' => $actor->getName(),
            'actorAvatar' => $actor->getAvatar(SOCIAL_AVATAR_SQUARE),
            'actorLink' => $actor->getPermalink(true, true),
            'comment' => $comment->comment
        ...
        // Standard email subject
        $ownerTitle = 'APP_PROJECT_PHOTOS_EMAILS_COMMENT_PHOTO_ITEM_SUBJECT';
        $involvedTitle = 'APP_PROJECT_PHOTOS_EMAILS_COMMENT_PHOTO_INVOLVED_SUBJECT';

        // If this item is multiple share on the stream, we need to get the photo id here.
        if ($comment->element == 'stream.project.upload') {
        ...
        // For single photo items on the stream
        if ($comment->element == 'photos.project.upload' || $comment->element == 'photos.project.add' || $comment->element == 'photos.project.uploadAvatar' || $comment->element == 'photos.project.updateCover') {
            // Get the photo object
            $photo = FD::table('Photo');
        ...
        if ($comment->element == 'photos.project.uploadAvatar') {
            $verb = 'uploadAvatar';

            $ownerTitle = 'APP_PROJECT_PHOTOS_EMAILS_COMMENT_PROFILE_PICTURE_ITEM_SUBJECT';
            $involvedTitle = 'APP_PROJECT_PHOTOS_EMAILS_COMMENT_PROFILE_PICTURE_INVOLVED_SUBJECT';
        }

        if ($comment->element == 'photos.project.updateCover') {
            $verb = 'updateCover';

            $ownerTitle = 'APP_PROJECT_PHOTOS_EMAILS_COMMENT_PROFILE_COVER_ITEM_SUBJECT';
            $involvedTitle = 'APP_PROJECT_PHOTOS_EMAILS_COMMENT_PROFILE_COVER_INVOLVED_SUBJECT';
        }
        ...
        // Get a list of recipients to be notified for this stream item
        // We exclude the owner of the note and the actor of the like here
        $recipients = $this->getStreamNotificationTargets($comment->uid, $element, 'project', $verb, $additionalRecipients, array($photo->user_id, $comment->created_by));

        $emailOptions['title'] = $involvedTitle;
        $emailOptions['template'] = 'apps/project/photos/comment.photo.involved';
