 ...
 */
class SocialProjectAppTasks extends SocialAppItem
{
    /**
    ...
    public function getFavIcon()
    {
        ...
        $obj->label = 'APP_PROJECT_TASKS_STREAM_TOOLTIP';
    ...
    {
        $allowed = array('tasks.project.createMilestone', 'task.project.createTask');
    ...
    {
        $allowed = array('tasks.project.createMilestone', 'tasks.project.createTask');
     ...
     * @return
     */
    public function onAfterDelete(SocialProject &$project)
    {
        $db = FD::db();
        $sql = $db->sql();
    ...
    /**
     * Processes notification for projects
     *
     * @since    1.2
     * @access    public
     * @param    string
    ...
    {
        $cmds = array('projects.milestone.created', 'projects.task.created', 'projects.task.completed', 'comments.item', 'comments.involved', 'likes.item', 'likes.involved');
        ...
        if (in_array($item->cmd, array('likes.item', 'likes.involved', 'comments.item', 'comments.involved')) && in_array($item->context_type, array('tasks.project.createMilestone', 'tasks.project.createTask'))) {
        ...
        if ($item->cmd === 'projects.task.completed') {
            ...
            $item->title = JText::sprintf('APP_PROJECT_TASKS_NOTIFICATIONS_USER_COMPLETED_TASK', $actor->getName(), $milestone->title);
            $item->content = $task->title;
        }

        if ($item->cmd === 'projects.task.created') {
            // Get the milestone data
            $id = $item->context_ids;
            $task = FD::table('Task');
            $task->load($id);
            ...
            $item->title = JText::sprintf('APP_PROJECT_TASKS_NOTIFICATIONS_USER_CREATED_TASK', $actor->getName(), $milestone->title);
            $item->content = $task->title;
        }

        if ($item->cmd === 'projects.milestone.created') {
            ...
            $item->title = JText::sprintf('APP_PROJECT_TASKS_NOTIFICATIONS_USER_CREATED_MILESTONE', $actor->getName(), $project->getName());
    ...
    public function onBeforeStorySave(&$streamTemplate, &$streamItem, &$template)
    {
        ...
        $project = FD::project($streamTemplate->cluster_id);
    ...
    public function onPrepareStoryPanel($story)
    {
        ...
        $project = FD::project($story->cluster);

        $tasks = FD::model('Tasks');
        $milestones = $tasks->getMilestones($project->id, SOCIAL_TYPE_PROJECT);
        ...
        $plugin->script = $script->output('apps:/project/tasks/story');

        // If there is no milestone, do not need to display the tasks embed in the story form.
        if (!$milestones) {
            $permalink = $this->getApp()->getPermalink('canvas', array('projectId' => $project->id, 'customView' => 'form'));
    ...
    public function onPrepareStream(SocialStreamItem &$item, $includePrivacy = true)
    {
        ...
        $item->label = FD::_('APP_PROJECT_TASKS_STREAM_TOOLTIP', true);
    ...
    public function prepareCreatedTaskStream(SocialStreamItem $streamItem, $includePrivacy = true)
    {
        ...
        $project->bind($params->get('project'));

        $app = $this->getApp();
        $permalink = FRoute::apps(array('layout' => 'canvas', 'customView' => 'item', 'uid' => $project->getAlias(), 'type' => SOCIAL_TYPE_PROJECT, 'id' => $app->getAlias(), 'milestoneId' => $milestone->id));
        ...
        $streamItem->addOgDescription(JText::sprintf('APP_PROJECT_TASKS_STREAM_OPENGRAPH_CREATE_TASK', $streamItem->actor->getName(), $milestone->title, $project->getName()));
    }
    ...
    public function prepareCreateMilestoneStream(SocialStreamItem $streamItem, $includePrivacy = true)
    {
        ...
        // Append the opengraph tags
        $streamItem->addOgDescription(JText::sprintf('APP_PROJECT_TASKS_STREAM_OPENGRAPH_CREATE_MILESTONE', $streamItem->actor->getName(), $project->getName()));
    }
}
