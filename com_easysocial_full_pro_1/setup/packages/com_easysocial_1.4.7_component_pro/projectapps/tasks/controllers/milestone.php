...
class TasksControllerMilestone extends SocialAppsController
{
    ...
    public function delete()
    {
        ...
        if (!$id || !$milestone->id || $milestone->uid != $project->id) {
            return $ajax->reject();
        }

        $milestone->delete();

        // @points: projects.milestone.delete
        $points = FD::points();
        $points->assign('projects.milestone.delete', 'com_easysocial', $milestone->user_id);
    ...
    public function confirmDelete()
    {
        ...
        // Check if the user is allowed to create a discussion
        if (!$project->getGuest()->isGuest() && !$user->isSiteAdmin()) {
            return $ajax->reject();
        }

        $theme = FD::themes();
        $contents = $theme->output('apps/project/tasks/views/dialog.delete');
    ...
    public function unresolve()
    {
        ...
        // Check if the user is allowed to create a discussion
        if (!$project->getGuest()->isGuest() && !$my->isSiteAdmin()) {
            FD::info()->set(JText::_('APP_PROJECT_TASKS_NOT_ALLOWED_HERE'), SOCIAL_MSG_ERROR);
    ...
    public function save()
    {
        ...
        // Get the project
        $projectId = JRequest::getInt('cluster_id', 0);
        $project = FD::project($projectId);
        ...
        $milestone->uid = (int) $project->id;
        $milestone->type = SOCIAL_TYPE_PROJECT;
        $milestone->state = SOCIAL_TASK_UNRESOLVED;
        ...
        if (!$id) {
            ...
            if ($params->get('notify_milestone', true)) {
                $project->notifyMembers('milestone.create', array('userId' => $my->id, 'id' => $milestone->id, 'title' => $milestone->title, 'content' => $milestone->getContent(), 'permalink' => $url));
            }

            // @points: projects.milestone.create
            // Add points to the user that updated the project
            $points = FD::points();
            $points->assign('projects.milestone.create', 'com_easysocial', $my->id);
        }

        FD::info()->set(JText::_('APP_PROJECT_TASKS_MILESTONE_CREATED'));
