...
class SocialProjectAppTasksHookNotificationLikes
{
    public function execute($item)
    {
        ...
        if ($verb == 'createMilestone') {
            ...
            if ($milestone->owner_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_TASKS_USER_LIKES_YOUR_MILESTONE', count($users)), $names);
                $item->content = $milestone->get('title');
                return $item;
            }

            // This is for 3rd party viewers
            $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_TASKS_USER_LIKES_USERS_MILESTONE', count($users)), $names, FD::user($milestone->owner_id)->getName());
            $item->content = $milestone->get('title');
        }
        ...
        if ($verb == 'createTask') {
            ...
            if ($task->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_TASKS_USER_LIKES_YOUR_TASK', count($users)), $names);

                return $item;
            }

            // This is for 3rd party viewers
            $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_TASKS_USER_LIKES_USERS_TASK', count($users)), $names, FD::user($task->user_id)->getName());
        }
