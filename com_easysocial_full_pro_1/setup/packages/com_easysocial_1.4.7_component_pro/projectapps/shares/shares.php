...
class SocialProjectAppShares extends SocialAppItem
{
    /**
    ...
    public function onNotificationLoad(SocialTableNotification &$item)
    {
        ...
        // We should only process items from project here.
        $share = FD::table('Share');
        $share->load($item->context_ids);

        if ($share->element != 'stream.project') {
            return;
        }
    ...
    public function getFavIcon()
    {
        ...
        $obj->color = '#e74c3c';
        $obj->icon = 'fa-refresh';
        $obj->label = 'APP_PROJECT_REPOST_STREAM_TITLE';
    ...
    public function onAfterStreamSave(SocialStreamTemplate &$streamTemplate)
    {
        ...
        $mailParams['title'] = 'APP_PROJECT_SHARES_EMAILS_USER_REPOSTED_YOUR_POST_SUBJECT';
        $mailParams['template'] = 'apps/project/shares/stream.repost';
    ...
    private function getHelper(SocialStreamItem $item, SocialTableShare $share)
    {
        ...
        // Get class name.
        $className = 'SocialProjectSharesHelper' . ucfirst($element);
    ...
    public function onPrepareStream(SocialStreamItem &$item, $includePrivacy = true)
    {
        ...
        $likes = FD::likes();
        $likes->get($uid, $element, $verb, SOCIAL_APPS_GROUP_PROJECT, $item->uid);
        $item->likes = $likes;

        // Attach comments to the stream
        $comments = FD::comments($uid, $element, $verb, SOCIAL_APPS_GROUP_PROJECT, array('url' => $helper->getLink()), $item->uid);
        ...
        $item->color = '#e74c3c';
        $item->label = JText::_('APP_PROJECT_REPOST_STREAM_TITLE');
