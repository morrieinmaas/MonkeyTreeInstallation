...
class SocialProjectAppVideos extends SocialAppItem
{
    ...
    /**
     * Determines if videos should be enabled for a given project
     *
     * @since   1.4
     * @access  public
     * @param   string
    ...
    public function onStreamCountValidation(&$item, $includePrivacy = true)
    {
        ...
        $project = ES::project($params->get('project'));
        ...
        if (!$project->isOpen() && !$project->isMember($this->my->id)) {
            $item->cnt = 0;
        }
    ...
    public function onPrepareStream(SocialStreamItem &$stream, $includePrivacy = true)
    {
        ...
        // Get the video
        $video = ES::video($stream->cluster_id, SOCIAL_TYPE_PROJECT, $stream->contextId);
        ...
        $stream->title = parent::display('themes:/site/videos/stream/project/title.' . $stream->verb);
    ...
    public function onPrepareStoryPanel(SocialStory $story)
    {
        ...
        // Get the project id
        $projectId = $story->cluster;

        // Ensure that the project allows users to upload videos
        $project = ES::project($projectId);

        $projectHasVideos = $project->getParams()->get('videos');
        ...
        $adapter = ES::video($projectId, SOCIAL_TYPE_PROJECT);
    ...
    public function onBeforeStorySave(SocialStreamTemplate &$template, SocialStream &$stream, $content)
    {
        ...
        if ($data['source'] == 'link') {
            $video = ES::video($template->cluster_id, SOCIAL_TYPE_PROJECT);
        }
        ...
        if ($data['source'] == 'upload') {
            ...
            $video = ES::video($template->cluster_id, SOCIAL_TYPE_PROJECT, $id);
    ...
    public function onAfterLikeDelete(&$likes)
    {
        ...
        if ($likes->type == 'videos.project.create' || $likes->type == 'videos.project.featured') {
    ...
    {
        $allowed = array('videos.project.create', 'videos.project.featured');
        ...
        $emailOptions = array(
            'template' => 'apps/project/videos/like.video.item',
            'actor' => $actor->getName(),
        ...
        if ($likes->type == 'videos.project.create') {
            $verb = 'create';
        }

        if ($likes->type == 'videos.project.featured') {
            $verb = 'featured';
        }
        ...
        $emailOptions['template'] = 'apps/project/videos/like.video.involved';
    ...
    {
        $allowed = array('project.video.create', 'comments.item', 'comments.involved', 'likes.item');

        if (!in_array($item->cmd, $allowed)) {
            return;
        }

        if ($item->cmd == 'project.video.create') {
    ...
    public function onAfterCommentSave(&$comment)
    {
        ...
        // Set the email options
        $emailOptions   = array(
            'template' => 'apps/project/videos/comment.video.item',
            'actor' => $actor->getName(),
            'actorAvatar' => $actor->getAvatar(SOCIAL_AVATAR_SQUARE),
            'actorLink' => $actor->getPermalink(true, true),
        ...
        // Get a list of recipients to be notified for this stream item
        // We exclude the owner of the note and the actor of the like here
        $recipients = $this->getStreamNotificationTargets($comment->uid, $element, 'project', $verb, array(), array($video->user_id, $comment->created_by));

        $emailOptions['title'] = $involvedTitle;
        $emailOptions['template'] = 'apps/project/videos/comment.video.involved';
