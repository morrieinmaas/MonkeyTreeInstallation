...
FD::import('apps:/project/links/helper');

class SocialProjectAppLinks extends SocialAppItem
{
    /**
    ...
    public function getFavIcon()
    {
        ...
        $obj->color = '#5580BE';
        $obj->icon = 'fa-link';
        $obj->label = 'APP_PROJECT_LINKS_STREAM_TOOLTIP';

        return $obj;
    }

    /**
     * Fixed legacy issues where the app is displayed on apps list of a project.
     *
     * @since   1.2
     * @access  public
     * @param   string
    ...
    /**
     * Generates the stream title of project.
     *
     * @since    1.0
     * @access    public
    ...
    public function onPrepareStream(SocialStreamItem &$stream, $includePrivacy = true)
    {
        ...
        $stream->label = FD::_('APP_PROJECT_LINKS_STREAM_TOOLTIP', true);

        // Apply likes on the stream
        $likes = FD::likes();
        $likes->get($stream->uid, $stream->context, $stream->verb, SOCIAL_APPS_GROUP_PROJECT, $stream->uid);
        $stream->likes = $likes;

        // Apply comments on the stream
        $comments = FD::comments($stream->uid, $stream->context, $stream->verb, SOCIAL_APPS_GROUP_PROJECT, array('url' => FRoute::stream(array('layout' => 'item', 'id' => $stream->uid))), $stream->uid);
        $stream->comments = $comments;

        // Apply repost on the stream
        $stream->repost = FD::get('Repost', $stream->uid, SOCIAL_TYPE_STREAM, SOCIAL_APPS_GROUP_PROJECT);
