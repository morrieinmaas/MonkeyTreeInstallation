...
class SocialProjectAppStoryHookNotificationComments
{
    /**
    ...
    public function execute(SocialTableNotification &$item)
    {
        ...
        // When someone comments on your status update in a project.
        if ($item->context_type == 'story.project.create') {
            ...
            if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_STORY_USER_POSTED_COMMENT_ON_YOUR_POST', count($users)), $names, $project->getName());

                return $item;
            }

            // This is for 3rd party viewers
            $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_STORY_USER_POSTED_COMMENT_ON_USERS_POST', count($users)), $names, FD::user($stream->actor_id)->getName(), $project->getName());

            return $item;
        }

        // When someone comments on the link you shared in the project
        if ($item->context_type == 'links.project.create') {
            ...
            if($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER)
            {
                $item->title    = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_STORY_USER_COMMENTED_ON_YOUR_LINK', count($users)), $names, $project->getName());

                return $item;
            }

            // This is for 3rd party viewers
            $item->title    = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_STORY_USER_COMMENTED_ON_USERS_LINK', count($users)), $names, FD::user($stream->actor_id)->getName(), $project->getName());
    ...
    private function notificationPhotos($names, $users, &$item)
    {
        ...
        if($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER)
        {
            $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_STORY_USER_COMMENTED_ON_YOUR_SHARED_PHOTO', count($users)), $names, $project->getName());

            return $item;
        }

        // This is for 3rd party viewers
        $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_STORY_USER_COMMENTED_ON_USERS_SHARED_PHOTO', count($users)), $names, FD::user($stream->actor_id)->getName(), $project->getName());
    }
}
