...
class SocialProjectAppStoryHookNotificationLikes
{
    /**
    ...
    public function execute(&$item)
    {
        ...
        // When someone likes your post in a project
        if ($item->context_type == 'story.project.create') {
            ...
            {
                $langString = FD::string()->computeNoun('APP_PROJECT_STORY_USER_LIKES_YOUR_POST', count($users));
            ...
            $langString = FD::string()->computeNoun('APP_PROJECT_STORY_USER_LIKES_USERS_POST', count($users));
            $item->title = JText::sprintf($langString, $names, FD::user($stream->actor_id)->getName(), $project->getName());
        ...
        if ($item->context_type == 'links.create') {
            ...
            if($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER)
            {
                $langString = FD::string()->computeNoun('APP_PROJECT_STORY_USER_LIKES_YOUR_LINK', count($users));
                $item->title = JText::sprintf($langString, $names, $project->getName());
            ...
            // This is for 3rd party viewers
            $langString = FD::string()->computeNoun('APP_PROJECT_STORY_USER_LIKES_USERS_LINK', count($users));
            $item->title = JText::sprintf($langString, $names, FD::user($stream->actor_id)->getName(), $project->getName());
    ...
    private function notificationPhotos($names, $users, &$item)
    {
        ...
        if($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER)
        {
            $langString = FD::string()->computeNoun('APP_PROJECT_STORY_USER_LIKES_YOUR_SHARED_PHOTO', count($users));
            $item->title = JText::sprintf($langString, $names, $project->getName());
        ...
        // This is for 3rd party viewers
        $langString = FD::string()->computeNoun('APP_PROJECT_STORY_USER_LIKES_USERS_SHARED_PHOTO', count($users));
        $item->title = JText::sprintf($langString, $names, FD::user($stream->actor_id)->getName(), $project->getName());
    }
}
