...
class SocialProjectAppFilesHookNotificationComments
{
    public function execute(&$item)
    {
        ...
        if ($item->context_type == 'files.project.uploaded') {
            ...
            if ($file->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_FILES_USER_COMMENTED_ON_YOUR_FILE', count($users)), $names);

                return $item;
            }

            // This is for 3rd party viewers
            $item->title = JText::sprintf(FD::string()->computeNoun('APP_PROJECT_FILES_USER_COMMENTED_ON_USERS_FILE', count($users)), $names, FD::user($file->user_id)->getName());
