 ...
 *
 */
class SocialPhotoAdapterProject extends SocialPhotoAdapter
{
	...
	public function __construct(SocialPhoto $lib , SocialAlbums $albumLib)
	{
		...
		$this->access = $this->project->getAccess();
	...
	{
		$url 	= FRoute::albums( array( 'layout' => 'item' , 'id' => $this->album->getAlias() , 'uid' => $this->project->getAlias() , 'type' => SOCIAL_TYPE_GROUP ) );
	...
	public function editable()
	{
		...
		if ($this->project->isAdmin() || $this->photo->user_id == $this->my->id) {
			return true;
		}
	...
	public function deleteable()
	{
		...
		// Project admins are allowed to delete photos
		if ($this->project->isAdmin()) {
			return true;
		}
	...
	/**
	 * Determines if the project exceeded their upload limits
	 *
	 * @since	1.3
	 * @access	public
	 * @param	string
	...
	public function exceededUploadLimit()
	{
		...
		if ($this->access->exceeded('photos.max', $this->project->getTotalPhotos())) {
			return true;
		}
	...
	/**
	 * Determines if the project exceeded their disk storage usage
	 *
	 * @since	1.3
	 * @access	public
	 * @param	string
	...
	{
		// Not implemented on projects yet
		return false;
	}

	/**
	 * Determines if the project exceeded their daily upload limit
	 *
	 * @since	4.0
	 * @access	public
	 * @param	string
	...
	public function exceededDailyUploadLimit()
	{
		...
		if ($this->access->exceeded('photos.maxdaily', $this->project->getTotalPhotos(true))) {
			return true;
		}
	...
	public function createStream( $verb, $mysqldatestring = '' )
	{
		...
		$tpl->setCluster($this->project->id, SOCIAL_TYPE_PROJECT);

		// Set the params to cache the group data
		$registry = FD::registry();
		$registry->set('project', $this->project);
	...
	{
		if ($this->photo->uid == $this->project->id) {
			return true;
		}
	...
	public function getDefaultAlbum()
	{
		...
		$album = $model->getDefaultAlbum($this->project->id, SOCIAL_TYPE_PROJECT, SOCIAL_ALBUM_PROFILE_PHOTOS);
