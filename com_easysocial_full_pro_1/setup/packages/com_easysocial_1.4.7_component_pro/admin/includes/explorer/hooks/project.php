...
class SocialExplorerHookproject extends SocialExplorerHooks
{
	...
	/**
	 * Determines if the project has ability to upload files here
	 *
	 * @since	1.3
	 * @access	public
	 * @param	string
	...
	public function allowUpload()
	{
		...
		$total = (int) $model->getTotalFiles($this->project->id, SOCIAL_TYPE_project);
	...
	/**
	 * Determines if the current person has access to the explorer of the project
	 *
	 * @since	1.3
	 * @access	public
	 * @param	string
	...
	public function hasReadAccess()
	{
		...
		if ($this->project->getGuest()->isGuest()) {
			return true;
		}
	...
	/**
	 * Determines if the user has access to delete the files on the project
	 *
	 * @since	1.3
	 * @access	public
	 * @param	string
	...
	public function hasDeleteAccess(SocialTableFile $file)
	{
		...
		if ($this->project->isAdmin() || $this->my->isSiteAdmin()) {
			return true;
		}
	...
	public function removeFolder($id = null)
	{
		...
		if (!$this->project->isMember() && !$this->my->isSiteAdmin()) {
	...
	/**
	 * Removes a file from an project.
	 *
	 * @since	1.3
	 * @access	public
	...
	public function addFile($title = null)
	{
		...
		if ($createStream) {
			...
			$tpl->setCluster($this->project->id, SOCIAL_TYPE_project, 1);
			...
			$this->project->notifyMembers('file.uploaded', array('fileId' => $file->id, 'fileName' => $file->name, 'fileSize' => $file->getSize(), 'permalink' => $permalink, 'userId' => $file->user_id));
		}
	...
	{
		return $this->project->canViewItem();
	}
}
