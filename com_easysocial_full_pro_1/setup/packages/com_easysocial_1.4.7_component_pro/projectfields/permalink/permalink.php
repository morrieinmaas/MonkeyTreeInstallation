...
FD::import('fields:/project/permalink/helper');

class SocialFieldsProjectPermalink extends SocialFieldItem
{
    /**
    ...
    public function onEditValidate(&$post, &$cluster)
    {
        ...
        if ($cluster->isRecurringProject() && !empty($post['applyRecurring'])) {
            return true;
        }
    ...
    public function validate($post, $cluster = null)
    {
        ...
        if ($this->isRequired() && empty($value)) {
            $this->setError(JText::_('FIELDS_PROJECT_PERMALINK_REQUIRED'));

            return false;
        }

        if ($this->params->get('max') > 0 && JString::strlen($value) > $this->params->get('max')) {
            $this->setError(JText::_('FIELDS_PROJECT_PERMALINK_EXCEEDED_MAX_LENGTH'));
            return false;
        }
        ...
        if (SocialFieldsProjectPermalinkHelper::exists($value)) {
            $this->setError(JText::_('FIELDS_PROJECT_PERMALINK_NOT_AVAILABLE'));

            return false;
        }

        if (!SocialFieldsProjectPermalinkHelper::valid($value, $this->params)) {
            $this->setError(JText::_('FIELDS_PROJECT_PERMALINK_INVALID_PERMALINK'));
    ...
    {
        // Support for recurring project
        // If this is a recurring project, then we do not want to process this
        if ($cluster->isRecurringProject() && !empty($post['applyRecurring'])) {
            unset($post[$this->inputName]);
            return true;
        }
