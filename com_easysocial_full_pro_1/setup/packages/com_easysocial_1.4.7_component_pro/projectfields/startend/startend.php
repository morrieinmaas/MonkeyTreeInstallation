...
class SocialFieldsProjectStartend extends SocialFieldItem
{
    public function onRegister(&$post, &$session)
    {
        ...
        if (isset($post['project_allday'])) {
            $allday = $post['project_allday'];
        }
    ...
    {
        // Get the start datetime
        $startDatetime = !empty($post['startDatetime']) ? $post['startDatetime'] : $node->getProjectStart()->toSql();
        $startDatetimeObj = $this->getDatetimeObject($startDatetime);
        $this->set('startDatetime', $startDatetimeObj->isValid() ? $startDatetimeObj->toSql() : '');

        // Get the end datetime
        $endDatetime = !empty($post['endDatetime']) ? $post['endDatetime'] : ($node->hasProjectEnd() ? $node->getProjectEnd()->toSql() : '');
        $endDatetimeObj = $this->getDatetimeObject($endDatetime);
    ...
    /**
     * Displays the start and end date in the project
     *
     * @since   1.3
     * @access  public
     * @param   string
    ...
    public function onDisplay($node)
    {
        ...
        if ($this->params->get('allow_time')) {
            $dateFormat = JText::_(FD::config()->get('projects.timeformat') == '12h' ? 'COM_EASYSOCIAL_DATE_DMY12H' : 'COM_EASYSOCIAL_DATE_DMY24H');
        }

        $this->set('dateFormat', $dateFormat);

        $startDatetime = $node->getProjectStart()->toSql();

        $startDatetimeObj = $this->getDatetimeObject($startDatetime);


        $end = $node->getProjectEnd();

        if ($node->meta->hasEnd()) {
            $endDatetime = $node->getProjectEnd()->toSql();
    ...
    {
        // Support for recurring project
        // If this is a recurring project, and is coming from applyRecurring, then we do not want to process this
        if ($node->isRecurringProject() && !empty($post['applyRecurring'])) {
            return true;
        }
    ...
    public function validate(&$post)
    {
        ...
        if ($startDatetimeObj->isEmpty()) {
            $this->setError(JText::_('FIELDS_PROJECT_STARTEND_VALIDATION_DATETIME_START_REQUIRED'));
            return false;
        }

        if (!$startDatetimeObj->isValid()) {
            $this->setError(JText::_('FIELDS_PROJECT_STARTEND_VALIDATION_DATETIME_INVALID_START'));
            return false;
        }

        if ($this->params->get('require_end') && $endDatetimeObj->isEmpty()) {
            $this->setError(JText::_('FIELDS_PROJECT_STARTEND_VALIDATION_DATETIME_END_REQUIRED'));
            return false;
        }

        if (!$endDatetimeObj->isEmpty() && !$endDatetimeObj->isValid()) {
            $this->setError(JText::_('FIELDS_PROJECT_STARTEND_VALIDATION_DATETIME_INVALID_END'));
            return false;
        }

        // End datetime must be > than start datetime
        if (!$endDatetimeObj->isEmpty() && $endDatetimeObj->toUnix() < $startDatetimeObj->toUnix()) {
            $this->setError(JText::_('FIELDS_PROJECT_STARTEND_VALIDATION_END_MUST_BE_LATER_THAN_START'));
            return false;
        }
    ...
    {
        // Timezone issue
        // Timezone disabled: Always assume everything is in UTC regardless of server config because user won't be able to change timezone on the project itself. Stored date is in UTC so that it can be used "as is".
        ...
        if ((!empty($timezone) && $timezone !== 'UTC') && $timezone != $node->getProjectTimezone()) {
    ...
    {
        $dateObj = new SocialFieldsProjectStartendObject;
...
class SocialFieldsProjectStartendObject
{
    public $year;
    public $month;
    public $day;
