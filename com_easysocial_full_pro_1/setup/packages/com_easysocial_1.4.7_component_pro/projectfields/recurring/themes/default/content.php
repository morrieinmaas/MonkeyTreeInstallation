...
?>
<div data-project-field-recurring class="form-horizonal">
    <div class="help-block">
        <select name="<?php echo $inputName; ?>[type]" data-recurring-type class="form-control input-sm es-recurring-select">
            <option value="none" <?php if (empty($value) || empty($value->type) || $value->type == 'none') { ?>selected="selected"<?php } ?>><?php echo JText::_('FIELD_PROJECT_RECURRING_TYPE_NONE'); ?></option>
            <option value="daily" <?php if ($value->type == 'daily') { ?>selected="selected"<?php } ?>><?php echo JText::_('FIELD_PROJECT_RECURRING_TYPE_DAILY'); ?></option>
            <?php /* Temporarily don't want weekly option
            <option value="weekly" <?php if ($value->type == 'weekly') { ?>selected="selected"<?php } ?>><?php echo JText::_('FIELD_PROJECT_RECURRING_TYPE_WEEKLY'); ?></option>
            */ ?>
            <option value="monthly" <?php if ($value->type == 'monthly') { ?>selected="selected"<?php } ?>><?php echo JText::_('FIELD_PROJECT_RECURRING_TYPE_MONTHLY'); ?></option>
            <option value="yearly" <?php if ($value->type == 'yearly') { ?>selected="selected"<?php } ?>><?php echo JText::_('FIELD_PROJECT_RECURRING_TYPE_YEARLY'); ?></option>
        </select>
    </div>
    ...
    <div class="help-block" data-recurring-end-block <?php if (empty($value->type) || $value->type === 'none') { ?>style="display: none;"<?php } ?>>
        <div class="form-inline">
            <div class="form-group ml-0 mr-5">
                <label>
                    <?php echo JText::_('FIELD_PROJECT_RECURRING_END'); ?>:
                </label>
            </div>
            <div class="form-group ml-0 mr-0">
    ...
    <div class="help-block alert alert-info-2 mt-10" style="display: none;" data-recurring-schedule-loading-block>
        <div class="fd-loading"><span><?php echo JText::_('FIELD_PROJECT_RECURRING_CHECKING'); ?></span></div>
    </div>
