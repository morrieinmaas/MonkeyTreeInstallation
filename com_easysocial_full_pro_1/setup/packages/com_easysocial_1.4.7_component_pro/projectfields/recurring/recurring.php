...
class SocialFieldsProjectRecurring extends SocialFieldItem
{
    ...
    public function onAdminEdit(&$post, &$project, $errors)
    {
        // Do not display if this is a child project
        if (!empty($project->parent_id)) {
            return;
        }

        $display = $this->onEdit($post, $project, $errors);

        $this->set('showWarningMessages', empty($project->id) ? 0 : 1);

        return $display;
    }

    public function onEdit(&$post, &$project, $errors)
    {
        // Do not display if this is a child project
        if (!empty($project->parent_id)) {
            return;
        ...
        $this->set('allday', $project->isAllDay());
    ...
    {
        // Registration won't be child project.

        return $this->onValidate($post);
    }

    public function onEditValidate(&$post, &$project)
    {
        // If this is a child project, we don't want to validate
        if (!empty($project->parent_id)) {
            return true;
        }
    ...
    public function onAdminEditValidate(&$post, &$project)
    {
        // If this is a child project, we don't want to validate
        if (!empty($project->parent_id)) {
            return true;
    ...
    private function onValidate(&$post)
    {
        ...
        if ($value->type !== 'none' && empty($value->end)) {
            $this->setError(JText::_('FIELD_PROJECT_RECURRING_VALIDATION_END_DATE_REQUIRED'));
            return false;
        }
        ...
        $endDatetimeObj = new SocialFieldsProjectStartendObject;
        $endDatetimeObj->load($endDatetime);

        $recurringEnd = FD::date($value->end);

        // Check if the end recur date is more than the project end date
        if ($recurringEnd->toUnix() < $endDatetimeObj->toUnix()) {
            $this->setError(JText::_('FIELD_PROJECT_RECURRING_VALIDATION_INVALID_END_DATE'));
            return false;
        }

        // If max is set to 0 then we don need to validate
        $max = FD::config()->get('projects.recurringlimit', 0);
        if (empty($max) || $max == 0) {
            return true;
        }

        $startDatetime = !empty($post['startDatetime']) ? $post['startDatetime'] : '';
        $startDatetimeObj = new SocialFieldsProjectStartendObject;
        $startDatetimeObj->load($startDatetime);

        $schedule = FD::model('Projects')->getRecurringSchedule(array(
            'projectStart' => $startDatetimeObj->toDate(),
            'end' => $value->end,
            'type' => $value->type,
            'daily' => $value->daily
        ));
        ...
        if ($totalRecurring > $max) {
            $this->setError(JText::sprintf('FIELD_PROJECT_RECURRING_VALIDATION_MAX_RECURRING_LIMIT', $totalRecurring, $max));
            return false;
        }
    ...
    public function onRegisterAfterSave(&$post, &$project)
    {
        // If this is a child project, we don't want the recur data.
        if (!empty($project->parent_id)) {
        ...
        if ($value->type !== 'none') {
            // Store the recurring data on the project object so that the controller can do subsequent processing in recurring project creation
            $project->recurringData = $value;
        }
    ...
    public function onEditAfterSave(&$post, &$project)
    {
        // If this is a child project, we don't want the recur data.
        if (!empty($project->parent_id)) {
            unset($post[$this->inputName]);
        ...
        // Detect changes to see if need to recreate projects
        $changed = false;

        // If there is no child projects and the type is not none, then it is considered as change
        if (!$project->hasRecurringProjects() && $value->type != 'none') {
            $changed = true;
        }
    ...
    public function getWeekdays()
    {
        ...
        // Configurable option
        $startOfWeek = FD::config()->get('projects.startofweek', 0);
    ...
    {
        $value = new SocialFieldsProjectRecurringObject($data);

        return $value;
    }
}

class SocialFieldsProjectRecurringObject
{
    public $type;
    public $end;
